...
This document defines implementation-neutral lifecycle behavior for the [domain model](domain-model.md). It is an approved design, not implemented functionality. State names are shown in `PascalCase`; commands are authorized FastAPI backend operations, even when initiated by the frontend or coordinated by n8n.

Unless a row explicitly says otherwise, a failed guard leaves canonical state unchanged, returns a conflict or validation result, and records a rejected-command audit event when the attempt is security-relevant or operationally material. Successful backend-controlled transitions and their audit events must commit in one database transaction.
...
| `AwaitingApproval` | `ApproveProposedAction` | Authorized approver; exact active proposal ID/version/digest; no prior effective decision | `ActionPendingExecution`; queue `Standard requests` for Low/Normal, `Priority requests` for High, or `Human review` for Urgent | Authorized approver through backend | `approval.approved`, `service_request.action_approved`, optional `service_request.queue_changed` | Authorization, stale version, or digest mismatch rejects without approval or execution. |
| `AwaitingApproval` | `RejectProposedAction` | Authorized approver; exact active proposal ID/version/digest; no prior effective decision | `ActionRevisionRequired`; queue `Human review` | Authorized approver through backend | `approval.rejected`, `service_request.action_revision_required` | No outbound attempt is created. Conflicting second decision is rejected. |
| `ActionRevisionRequired` | `SubmitRevisedAction` | Authorized operator; new proposal version/digest; prior proposal remains rejected or superseded | `AwaitingApproval`; queue `Human review` | Authorized operations user through backend | `proposed_action.version_created`, `proposed_action.submitted`, `service_request.awaiting_approval` | Reusing the old version or approval is rejected. |
...
| `RetryableFailure` | `MarkTerminalFailure` | Authorized disposition or retry policy exhausted; reason recorded | `TerminalFailure`; queue `Failed/retry required` | Authorized operations/administrator through backend | `service_request.terminal_failure` | Missing authority/reason leaves state unchanged. |
...
| `Draft` or `PendingApproval` | `CreateMaterialRevision` | Authorized operator; no running/successful attempt | Old version `Superseded`; new version `Draft` | Authorized operations user through backend | `proposed_action.superseded`, `proposed_action.version_created` | Both version changes commit together; no approval transfers. |
| `PendingApproval` | `ApproveProposedAction` | Authorized approver; exact ID/version/digest; no existing effective decision | `Approved` | Authorized approver through backend | `approval.approved`, `proposed_action.approved` | Stale digest, unauthorized actor, or prior decision leaves state unchanged. |
| `PendingApproval` | `RejectProposedAction` | Authorized approver; exact ID/version/digest; no existing effective decision | `Rejected` | Authorized approver through backend | `approval.rejected`, `proposed_action.rejected` | No integration attempt can be created. |
...
| `Approved` | `CreateMaterialRevision` | Authorized operator; no running/successful attempt | Old version `Superseded`; new version `Draft`; prior approval becomes ineffective for execution | Authorized operations user through backend | `proposed_action.superseded`, `approval.execution_validity_lost`, `proposed_action.version_created` | Atomic failure retains old state; execution is blocked during revision command. |
...
| `RetryableExecutionFailure` | `CreateMaterialRevision` | Authorized operator; no active/successful attempt | Old version `Superseded`; new version `Draft` requiring approval | Authorized operations user through backend | `proposed_action.superseded`, `proposed_action.version_created` | Old approval cannot authorize the revision. |
| `RetryableExecutionFailure` | `MarkTerminalExecutionFailure` | Retry policy exhausted or authorized disposition with reason | `TerminalExecutionFailure` | Backend recovery policy or authorized operator | `proposed_action.terminal_execution_failure` | Missing guard leaves state unchanged. |
| `Rejected` | `CreateRevisionAfterRejection` | Authorized operator; same proposal family; new version and digest | Existing proposal remains `Rejected`; new version is `Draft` | Authorized operations user through backend | `proposed_action.version_created` | Historical rejection is never rewritten. |
...
| Failed/retry required | `InboundDelivery` and `ServiceRequest` | Delivery `ProcessingFailure`; request `RetryableFailure` or `TerminalFailure` requiring visibility | Derived for deliveries; persisted for requests. Retry eligibility is read from failure facts, not inferred from queue name. |

Queue precedence is: failure, duplicate review, human/approval/Urgent review, then priority-based Standard or Priority. After approval, `ActionPendingExecution` uses Standard for Low/Normal, Priority for High, and Human review for Urgent oversight. Initial `TriagePending` processing and terminal `Completed`/`ClosedDuplicate` records may have no active queue. Queue changes on service requests produce `service_request.queue_changed` with old/new queue, reason, rule version, actor, and correlation ID. Priority changes produce separate decision evidence; moving queues never silently changes priority.
...
14. Material changes to an approved proposal create a new version and invalidate the old approval for execution.
...
| 12 | Rejected outbound action | Request `AwaitingApproval`; proposal `PendingApproval` | Authorized approver rejects exact ID/version/digest | Request `ActionRevisionRequired`; proposal `Rejected` | Human review | Rejection decision, proposal/request state change; no adapter event | No |
