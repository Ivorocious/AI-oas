...
| `RetryableFailure` | `MarkTerminalFailure` | Authorized disposition or retry policy exhausted; reason recorded | `TerminalFailure`; queue `Failed/retry required` | `ManagerApprover` or `Administrator` through backend | `service_request.terminal_failure` | Missing authority/reason leaves state unchanged. |
