...
| **Implemented internally; no public route:** Complete deterministic triage — trusted `CompleteTriage` intent | Expected `service_request`; references only to current stored interpretation and duplicate evidence; optional expected policy identity | Request `TriagePending`; validated evidence and selected policy current; `BackendService` deterministically creates candidates and calculates final category, priority, status, queue, and review codes | Atomic service result contains routing-decision ID, policy ID/version/digest, category, priority, status, queue, ordered review codes, and request version |
...
| Approve exact proposal — `POST /api/v1/proposed-actions/{action_id}/commands/approve` | Expected `service_request` and `proposed_action`; exact payload digest; optional rationale | Request `AwaitingApproval`; proposal `PendingApproval`; distinct `ManagerApprover` or `Administrator`; actor absent from frozen attribution exclusion; no prior decision; digest exact | Command key required; `200 OK` with exact immutable `approval_decision_id`, proposal `Approved`, request `ActionPendingExecution`, queue, versions; replay retains the decision ID |
| Reject exact proposal — `POST /api/v1/proposed-actions/{action_id}/commands/reject` | Expected `service_request` and `proposed_action`; exact payload digest; rationale when required | Same distinct-approver, self-decision, exact-version/digest, and no-prior-decision guards as approval | Command key required; `200 OK` with exact immutable `approval_decision_id`, proposal `Rejected`, request `ActionRevisionRequired`, queue, versions; replay retains the decision ID; no attempt |
| Create material revision — `POST /api/v1/proposed-actions/{action_id}/commands/create-material-revision` | Expected `service_request` and `proposed_action`; replacement draft content | Source/request pair must match an allowed revision path; no active/successful attempt or successful operation; authorized operations actor. Backend requires and preserves the same request, proposal series, and existing outbound logical operation; it creates no operation | Command key required; `201 Created` with `source_proposed_action_id`, source state, `replacement_proposed_action_id`, replacement state, unchanged series/operation IDs, `recovery_cleared`, and versions; no approval transfer |
