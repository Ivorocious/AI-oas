...
Durable callback-command replay also needs to remember which exact credential established authority after that credential is consumed. The dedicated callback-authorization binding foundation now records that credential ID/version independently from one-time secret-delivery metadata. It does not imply plaintext issuance, expose the binding in an API response, or implement a callback route.
