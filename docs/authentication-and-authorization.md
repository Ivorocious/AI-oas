...
> Implementation status: accepted Checkpoint 4 commit `4735ce9d78f2f912d7ad93060a1589f138183052` implements the outbound authentication boundary. The locally validated Checkpoint 5 candidate adds all thirteen protected query operations and remains uncommitted, unpushed, and pending Orchestration acceptance. BackendService remains in-process only; no reusable external BackendService credential was added.

## Status and scope

This document defines the fixed MVP identity, authentication, and authorization model. Human access and WorkflowService HMAC, nonce, command-idempotency, and attempt-scoped callback authentication protect the implemented boundaries. The assigned WorkflowService may start only its exact `Pending` AI or mock-outbound attempt. Callbacks require HMAC plus the exact opaque credential inside the transaction, with constrained AI/outbound retry failing closed when credentials are missing or ambiguous. Trusted in-process BackendService authority executes deterministic triage and stale/deadline assessment; it is not exposed as a reusable credential or public route. EventPublisher execution, controlled rotation operations, and hosted secret-manager integration remain unimplemented.
...
The production callback authorizer implements this proof inside a caller-owned explicit transaction. It locks the exact attempt, operation, owner request, sibling attempts, and all callback-credential rows. First execution requires the one Active highest-version credential matching the assigned stable WorkflowService identity/environment and PostgreSQL-controlled expiry. A Consumed highest-version credential authorizes only the exact completed command replay whose durable binding matches it; Replaced and Revoked credentials never authorize. Plaintext is never stored, returned in the safe context, or used in SQL lookup predicates.

Durable callback-command replay remembers which exact credential established authority after that credential is consumed. The callback-authorization binding records that credential ID/version independently from one-time secret-delivery metadata. It does not imply plaintext issuance or expose the binding in an API response.
...
The catalog has 21 documented command intents. Twenty are external HTTP intents: submission and replacement submission deliberately share one route, yielding nineteen external mutation operations and nineteen unique external mutation path templates. The twenty-first intent is trusted in-process `CompleteTriage`, which has no HTTP route. Including that internal intent gives twenty normalized mutation templates in the authorization catalog. OpenAPI independently contains thirteen protected query operations and `/health`.
...
Human authentication, current-role authorization, reusable WorkflowService HMAC, nonce, command idempotency, attempt-scoped callback authentication, durable callback-command authorization binding, credential replacement, AI and mock-outbound callbacks/retries, deterministic triage/review, proposal approval, and all thirteen protected query operations are implemented. Start commands issue one created-attempt callback credential after commit, and the assigned WorkflowService can claim/start only that exact eligible attempt. Hosted secret-manager integration, controlled rotation operations, EventPublisher execution, demo provisioning, real providers, frontend, deployment, and Phase 3 remain deferred.
